...
import type { Category, Document } from "@/api/types";
import {
  getDocument,
  createDocument,
  updateDocument,
...
const isEdit = computed(() => !!getRouteID());
...
const loadDocument = async () => {
  ...
  if (!routeID) return;
  try {
    const res = await getDocument(routeID);
    const doc = res.data;
    form.title = doc.title || "";
    form.categoryID = doc.categoryID || null;
    form.tags = doc.tags?.map((t: { name: string }) => t.name) || [];
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value && routeID) {
      await updateDocument(routeID, data);
      showSnackbar("更新成功");
    } else {
